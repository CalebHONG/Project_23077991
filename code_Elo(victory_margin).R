calculate_victory_margin <- function(first_innings, winner, team1, team2, team1_runs, 
                                     team2_runs, team1_balls, team2_balls, team1_wickets, team2_wickets) {
  
  if (first_innings == winner) {
    if (first_innings == team1) {
      return(max(team1_runs - team2_runs,10))
    } else {
      return(max(team1_runs - team2_runs,10))
    }
  } else {
    if (first_innings == team1) {
      return(max(-((team2_runs-team1_runs)*
               ((125 - team2_balls)+( 10 - team2_wickets))
             /( (125 - team1_balls)+(10 - team1_wickets))),10))
      #return(-(team2_runs-team1_runs)-mean(c((120 - team2_balls),( 10 - team2_wickets))))
    } else {
      return(max((team1_runs-team2_runs)*
                 ((125 - team1_balls)+( 10 - team1_wickets))
               /( (125 - team2_balls)+(10 - team2_wickets)),10))
      #return((team1_runs-team2_runs)+ mean(c((120 - team1_balls),(10 - team1_wickets))))
    }
  }
}
...
for (i in 1:544){
bbl_data$victory_margin[i] <- calculate_victory_margin(
  first_innings = bbl_data$first_innings[i], 
  winner = bbl_data$winner[i], 
  team1  = bbl_data$team1[i], 
  team2  =bbl_data$team2[i], 
  team1_runs =bbl_data$runs_team1[i], 
  team2_runs = bbl_data$runs_team2[i], 
  team1_balls   =bbl_data$balls_team1[i], 
  team2_balls = bbl_data$balls_team2[i], 
  team1_wickets  =bbl_data$wickets_team1[i], 
  team2_wickets=  bbl_data$wickets_team2[i])
}
##################################################################################################
############################
###########################################
...
expected_win_probability <- function(home_elo, away_elo, theta = 400, home_advantage = 80, 
                                     is_home = 1) {
  if (is_home == 1) {
...
update_elo <- function(home_elo, away_elo, home_win, margin_of_victory, K = 40, theta = 400, home_advantage = 80, is_home = 1) {
  exp <- expected_win_probability(home_elo, away_elo, theta, home_advantage, is_home)

  adjusted_K <- K * log(1 + abs(margin_of_victory))
  ...
  new_home_elo <- home_elo + adjusted_K * (obs_home - exp[1])
  new_away_elo <- away_elo + adjusted_K * (obs_away - exp[2])
...
for (i in 1:nrow(bbl_data)) {
  ...
  winner <- bbl_data$winner[i]
  is_home <- bbl_data$home[i]  # 变量 home
  ...
  margin_of_victory <- bbl_data$victory_margin[i]
  

  home_win <- (home_team == winner)
  new_elo <- update_elo(home_elo, away_elo, home_win, margin_of_victory, home_advantage = 80, is_home = is_home)
